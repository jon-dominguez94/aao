...
function balancedParens(str) {
  const parens = [];
  const { openings, closings } = getChars();

  for(let i = 0; i < str.length; i++){
    if(openings.includes(str[i])){
      parens.push(str[i]);
    } else if(closings.includes(str[i])){
      const check = checkMatch(parens, str[i]);
...
function getChars(){
  const types = "([{)]}";
  const valids = types.split("");
  const openings = valids.slice(0, 3);
  const closings = valids.slice(3);
  return { openings, closings };
}

function checkMatch(parens, char){
  const last = parens.pop();
  switch(char){
    case ')':
      return last === '(';
    case ']':
